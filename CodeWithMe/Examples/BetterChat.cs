...
namespace Oxide.Plugins
{
    ...
    internal class BetterChat : CovalencePlugin
    {
        ...
        {
            if (chatchannel == Chat.ChatChannel.Team) return null;
        ...
        public class BetterChatMessage
        {
            ...
            public ChatGroup.FormatSettings GetOutput()
            {
                ...
                var id = BasePlayer.FindAwakeOrSleeping(Player.Name).userID;
                if ((bool) _instance.Clans.Call("PlayerHaveClan", id))
                {
                    replacements["Username"] =
                        $"[#{UsernameSettings.GetUniversalColor()}][+{UsernameSettings.Size}]{Username}[/+][/#]".Replace($"[{(string) _instance.Clans?.Call("GetTag", id)}]", $"<color={(string) _instance.Clans?.Call("GetColor", id)}>[{(string) _instance.Clans?.Call("GetTag",id)}]</color>");
                }
