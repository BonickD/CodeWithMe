...
namespace Oxide.Plugins
{
    ...
    public class OurServers : RustPlugin 
    {
        ...
        private void CmdShowServers(BasePlayer player, string command, string[] args)
        {
            ...
            if (Settings.UseChat)
            {
                string resultMessage = Settings.Servers.Aggregate("", (current, server) => current + ($"{server.DisplayName} (IP: {server.IPAndPort}) - " + ((server.Status) ? $"{server.GetOnline()}/{server.MaxOnline} players online" : "OFFLINE") + "\n\n"));
